...
- **`Genetics`** is immutable (final fields, no setters) — this encodes "genetics cannot change after game start" as a type-level invariant rather than a runtime check. Holds `endomorphy`, `mesomorphy`, `ectomorphy` (three independent 0–10 axes, Heath-Carter inspired — not mutually exclusive categories), `height` (cm), `limbRatio` (~0.85–1.15), `boneDensity` (0–10), `muscleDistribution` (0–10, added 2026-06-29 — see below). Defaults: all somatotype/density axes at 5, height 170cm, limbRatio 1.0, `muscleDistribution` 5 (balanced).
- **`muscleDistribution` (added 2026-06-29, `task/movement-attributes-muscle-distribution`):** a leg-biased (low) to arm-biased (high) axis around the balanced midpoint (5) — the one `Genetics` axis besides the somatotype/density fields that feeds the output formulas directly rather than only modulating training rate. `Biomechanics.getMuscleDistributionDeviation()` (private) computes the −5..+5 deviation from the midpoint that both effects below scale from. Trade-off, per explicit user design: arm-bias gives a *small* `getStrength()` bonus (leg-bias a small penalty), and leg-bias gives a *larger* `getMaxMovementSpeed()` bonus (arm-bias a larger penalty) — opposite directions, deliberately asymmetric magnitudes (see `BiomechanicsBalance` exception below). `getSpeed()` itself is unchanged and stays the generic movement-capable speed used by any movement-involving action (including attacks); `getMaxMovementSpeed()` is the new derived attribute for max displacement/travel speed specifically.
...
- `attributes.maxMovementSpeed` (added 2026-06-29, alongside `genetics.muscleDistribution`) is exposed on `AttributesResponse`/`GeneticsResponse` — since both `BiomechanicsResponse.from()` and `BiomechanicsPreviewResponse.from()` already delegate to `AttributesResponse.from(biomechanics)`, this single change reaches both REST surfaces (character sheet and the preview endpoint below) without touching either wrapper response type.
...
*Last updated: 2026-06-29 (documented the `muscleDistribution` genetic trait, `getMaxMovementSpeed()`, and the two new `BiomechanicsBalance` coefficients from PR #16 — rpg-1..rpg-6)*