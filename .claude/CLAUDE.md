...
### Character sheet (implemented — restructured in rpg-8, 2026-06-30; contract changed again in rpg-11, 2026-07-01)

- `GET /api/v1/characters/{id}` returns a `CharacterResponse` (`id`, `name`, nested `body`) — first REST surface in this project.
- **`body` response structure:** `body.biomechanics` (genetics + bodyComposition), `body.bodySystems` (bloodSystem + cardiacSystem + pulmonarySystem + nervousSystem), `body.spatialIntelligence` (perception + agility + precision), `body.attributes`, `body.calculatedValues`, `body.loadCapacity` (new in rpg-11), `body.geneticPoints`, `body.trainingPoints`.
- **`body.attributes`** (rpg-11): `strength`, `speed`, `maxMovementSpeed`, `staminaPool`, `fatigueResistance`, `staminaRecovery`, `durability`, `sight`, `hearing`, `smell`, `evasion`, `acrobatics`, `meleeAccuracy`, `aim` — 14 fields, all additive-standard (baseline 60). `cardiovascularCapacity` and `fatigueRate` were **removed** (breaking change — see `.claude/skills/additive-attribute-formulas.md`); `fatigueResistance` (inverted semantics) and `staminaRecovery` (new) replace/extend them. `energyCost` remains excluded (real-time activity, not a static trait).
- **`body.calculatedValues`** (rpg-11): `symbolicTotalMass` (int), `displayMassKg` (double) — replaces the old `totalMass`/`boneMass`/`organWaterMass` triplet.
- **`body.loadCapacity`** (new in rpg-11): `lightLoadKg`, `heavyLoadKg`, `maxCapacityKg`, `dragCapacityKg` — all derived from `Strength`.
- **`bodyComposition`** carries `muscleDistribution` and `flexibility`. `genetics` no longer carries `muscleDistribution`.
- **`nervousSystem`** carries both `neuralDrive` and `neuromuscularEfficiency` — both ints, 1-9 (rpg-11; `neuromuscularEfficiency` was a 0-1 float before).
...
### Biomechanics preview (implemented — restructured in rpg-8, 2026-06-30; contract changed again in rpg-11, 2026-07-01)

- `POST /api/v1/biomechanics/preview` takes a `BiomechanicsPreviewRequest` with **4 groups**: `genetics`, `bodyComposition`, `bodySystems` (bloodSystem + cardiacSystem + pulmonarySystem + nervousSystem), `spatialIntelligence` (perception + agility + precision). All leaf fields are `int` as of rpg-11 (see the additive-standard subsection above for the new ranges).
- Response: `BiomechanicsPreviewResponse` nests `attributes`, `calculatedValues` (`symbolicTotalMass`, `displayMassKg`), and `loadCapacity` (new in rpg-11) — same shapes as the character sheet response.
