...
- `GET /api/v1/characters/{id}` returns a `CharacterResponse` (`id`, `name`, `body`, `mind`, `attributes`, `attributeBreakdowns`, `calculatedValues`, `loadCapacity`) — first REST surface in this project. **`attributes`/`attributeBreakdowns`/`calculatedValues`/`loadCapacity` moved up from nested under `body` to top-level siblings of `body`/`mind` in rpg-18** — they were always a whole-`PlayableCharacter` concern, and now that Mind-driven formulas exist this stopped being a cosmetic-only nesting choice.
- **`body` response structure (rpg-18: attributes/etc. moved out, see above):** `body.biomechanics` (genetics + bodyComposition), `body.bodySystems` (bloodSystem + cardiacSystem + pulmonarySystem + neuralSystem + hormonalSystem + digestiveSystem), `body.physicalTraits` (sensorialOrgans + bodyStructure, rpg-14), `body.geneticPoints`, `body.trainingPoints`. **`body.spatialIntelligence` no longer exists (rpg-13)** — its three fields moved into `body.bodySystems.neuralSystem`.
- **`mind` response structure (new, rpg-18):** `mind.values` (14 fields, 0-5), `mind.erudition` (`selectedTraits` — set of trait names, `freeTraitSlots` — currently 2), `mind.eventPoints` (same `{total, spent, remaining}` shape as `geneticPoints`/`trainingPoints`).
- **`attributes`** (rpg-11, expanded rpg-13/rpg-14/Delta V4/rpg-18): 66 fields. `strength` is **gone** (Delta V4) — replaced by `pushStrength`, `legDrive`, `gripStrength`, `liftStrength`, `swingPower`, `grapplingSelfLifting`; `sixthSense` **renamed to `mediunity`** (rpg-18, pure rename — no `sixthSense` field remains). New in rpg-18: 14 Concern fields (`selfConcern`...`peaceConcern`) and 9 Mind-driven attributes (`survivalSkills`, `animalCaring`, `manipulation`, `behaviorReading`, `discretion`, `bluffing`, `faith`, `illusionResistanceSanity`, `creativity`). `cardiovascularCapacity`, `fatigueRate`, `energyCost` remain absent (see rpg-11 removal note above). `meanStrength` (hidden engine) is never in this response.
- **`attributeBreakdowns`** (Delta V4, extended rpg-18): `Map<String, AttributeBreakdownResponse>` keyed the same as `attributes`, each `{baseline, terms}`. Absent for `swingPower`/`grapplingSelfLifting` (not additive-standard formulas) and for the Load Capacity group (separate DTO, non-linear transform).
- **`calculatedValues`**: `symbolicTotalMass` (int), `displayMassKg` (double). **`loadCapacity`**: `lightLoadKg`, `heavyLoadKg`, `maxCapacityKg`, `dragCapacityKg` — all `int` (rpg-12), derived from `liftStrength` (Delta V4).
...
- **`neuralSystem`** (renamed from `nervousSystem`, rpg-13) carries all eleven fields — `neuralDrive`, `neuromuscularEfficiency`, `cerebralCapacity`, `synapsisQuality`, `hippocampus`, `thalamus` (new, Delta V4), `hypothalamus`, `amygdalaAndCingulum`, `immunity`, `agility`, `precision`.
...
- `POST /api/v1/character/preview` (was `/api/v1/biomechanics/preview`) takes a `CharacterPreviewRequest { body: BodyPreviewRequest, mind: MindPreviewRequest }`. `BodyPreviewRequest` (renamed from `BiomechanicsPreviewRequest` — same 4 fields: `genetics`, `bodyComposition`, `bodySystems`, `physicalTraits`). `MindPreviewRequest { values: ValuesInput (14 fields), erudition: EruditionInput (selectedTraits: Set<String>) }`. All leaf fields are `int`.
- Response: `CharacterPreviewResponse` (renamed from `BiomechanicsPreviewResponse`, same shape) nests `attributes` (66 fields), `attributeBreakdowns`, `calculatedValues`, and `loadCapacity`.
- **Why this changed:** once `ShortMemory`/`Reasoning`/`Enfactuation`/`Will` started reading `Values` (rpg-18), a Body-only preview could no longer resolve every attribute correctly — the preview contract had to accept both pillars at once.
- **Stateless** — no persistence, no character identity. Not the eventual creation contract.
- `PreviewAttributesService` builds a transient `Body.previewTemplate(...)` and `Mind.previewTemplate(values, erudition)` and wraps both in a `PlayableCharacter` — `BodyCoefficients.defaults()` apply since none of the formulas depend on tuned coefficients for preview purposes.
- `PreviewAttributesUseCase.calculate(...)` gained two parameters (`Values`, `Erudition`) — this is a breaking signature change from the pre-rpg-18 3-parameter version.
