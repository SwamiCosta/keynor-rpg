...
### Character sheet (implemented — restructured in rpg-8, 2026-06-30; contract changed again in rpg-11/rpg-13)

- `GET /api/v1/characters/{id}` returns a `CharacterResponse` (`id`, `name`, nested `body`) — first REST surface in this project.
- **`body` response structure:** `body.biomechanics` (genetics + bodyComposition), `body.bodySystems` (bloodSystem + cardiacSystem + pulmonarySystem + neuralSystem + hormonalSystem + digestiveSystem), `body.attributes`, `body.calculatedValues`, `body.loadCapacity`, `body.geneticPoints`, `body.trainingPoints`. **`body.spatialIntelligence` no longer exists (rpg-13)** — its three fields moved into `body.bodySystems.neuralSystem`.
- **`body.attributes`** (rpg-11, expanded rpg-13): 29 fields — the 14 additive-standard attributes from rpg-11 (`strength`, `speed`, `maxMovementSpeed`, `staminaPool`, `fatigueResistance`, `staminaRecovery`, `durability`, `sight`, `hearing`, `smell`, `evasion`, `acrobatics`, `meleeAccuracy`, `aim`) plus 15 new rpg-13 fields (`memoryPool`, `reasoning`, `shortMemory`, `mentalHealthPool`, `will`, `balance`, `stressResistance`, `poisonResistance`, `diseaseResistance`, `bleedingResistance`, `thermalResistance`, `breathOutput`, `dehydrationResistance`, `starvationResistance`, `foodPoisoningAlcoholResistance`). `cardiovascularCapacity`, `fatigueRate`, `energyCost` remain absent (see rpg-11 removal note above).
...
- **`bodyComposition`** carries `muscleDistribution` and `flexibility`. `genetics` carries `skinThickness` (rpg-13) in addition to the rpg-11 fields; no longer carries `muscleDistribution`.
...
- **`hormonalSystem`** (new, rpg-13): `thyroid`, `adrenalGlands`. **`digestiveSystem`** (new, rpg-13): `nutrientAbsorption`, `impurityCleaning`, `ketosisQuality`.
...
### Biomechanics preview (implemented — restructured in rpg-8, 2026-06-30; contract changed again in rpg-11/rpg-13)

- `POST /api/v1/biomechanics/preview` takes a `BiomechanicsPreviewRequest` with **3 groups (rpg-13, was 4)**: `genetics`, `bodyComposition`, `bodySystems` (bloodSystem + cardiacSystem + pulmonarySystem + neuralSystem + hormonalSystem + digestiveSystem). `spatialIntelligence` no longer exists as a request field — its inputs moved into `bodySystems.neuralSystem`. All leaf fields are `int`.
- Response: `BiomechanicsPreviewResponse` nests `attributes` (29 fields), `calculatedValues` (`symbolicTotalMass`, `displayMassKg`), and `loadCapacity` — same shapes as the character sheet response.
- **Stateless** — no persistence, no character identity. Not the eventual creation contract.
- `PreviewAttributesService` builds a transient `Body.previewTemplate(biomechanics, bodySystems)` (2-arg since rpg-13 — `SpatialIntelligence` param removed) and wraps it in a `PlayableCharacter` — `BodyCoefficients.defaults()` apply since none of the formulas depend on tuned coefficients for preview purposes.
