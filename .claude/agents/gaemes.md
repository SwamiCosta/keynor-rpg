...
- Call `POST /api/v1/clown/chat` (or the real Anthropic API it proxies), for any reason including testing or verification — see `CLAUDE.md`'s "Clown chat endpoint (planned) — human-only invocation" section. Every call must come from a human via the UI's consent-checkbox-gated chat flow, with no exception for any agent at any level, including this one
