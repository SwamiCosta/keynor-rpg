...
- `.claude/skills/special-attack-test.md` — the d20 Special Attack Test mechanic, the four attack-type resolvers (melee/thrown/firearm/bow) and their weapon/armor reference-data enums, the Evasion reaction (`EvasionResolver`), and the Vb×Md×Dm protection/final-damage formulas. Open it before touching `SpecialAttackTestResolver`, any `*AttackResolver`, `EvasionResolver`, `*AttackProfile`/`*WeaponProfile`, `ArmorProtection`, `ProtectionCalculator`, or `DamageResolver` — including wiring any of them into a real use case for the first time.
...
- **Localize every new `AttributeBreakdown.Term` label (i18n, rpg-23).** Any new term label added to a `PlayableCharacter` formula needs a matching English→Portuguese entry added to `TermLabelTranslations` (`application.dto`) in the *same* delta — see `.claude/skills/additive-attribute-formulas.md`'s Localization section. This is the only backend-sourced text in this project; a missing entry silently falls back to English for `PT` requests instead of erroring, so it won't break a build, but it is still a real bug to fix before considering the PR done.
...
*Last updated: 2026-07-23 (task/remove-clown-chat-endpoint: removed the "Not permitted" bullet about `POST /api/v1/clown/chat` — that endpoint was removed at explicit user request; see `keynor-rpg/CLAUDE.md`'s changelog. Matching removals in `gaemes.md`/`clown.md` in the same delta.) Previous entry, 2026-07-21 (task/clown-chat-human-only-invocation: added a "Not permitted" bullet blocking this agent from ever calling the planned `POST /api/v1/clown/chat` endpoint, even to verify its own implementation — see `CLAUDE.md`'s new "Clown chat endpoint (planned) — human-only invocation" section. Matching restrictions added to `gaemes.md`/`clown.md` in the same delta.) Previous entry, 2026-07-20 (task/special-attack-test-mechanic: added `.claude/skills/special-attack-test.md` to Project skills — the new d20 Special Attack Test mechanic, its four attack-type resolvers, and the weapon/armor reference-data enums, none yet wired into a real use case.) Previous entry, 2026-07-12 (rpg-23, i18n: added a Responsibilities bullet requiring every new `AttributeBreakdown.Term` label to get a matching `TermLabelTranslations` (Portuguese) entry in the same delta — see `.claude/skills/additive-attribute-formulas.md`'s new Localization section. This is a cross-project multi-language initiative, already applied to `keynor-core`/`aniannoth-overview`; scope for this repository is narrow by explicit user decision — only content already backend-owned (term labels) gets a translation, nothing moved between layers.) Previous entry, 2026-07-04 (rpg-18: added `.claude/skills/mind-pillar-traits-and-values.md` to Project skills. Previous entry, same day — rpg-18 (formulas-reference-page-docs): added a Responsibilities bullet requiring any formula/coefficient/input change to be flagged to Gaemes, so keynor-rpg-client's in-app Formulas reference page stays in sync — see that project's formulas-reference-page.md skill. Previous entry, 2026-06-29: corrected Mandatory reading core to match the current SKILLS.md table — removed Skill 05, which is Never for this role; added missing Always skills 06/11/12/13; renamed "Level 2 (dev) column" to "Void column" to match the table's per-persona restructure)*
