...
- Call `POST /api/v1/clown/chat` (or the real Anthropic API), even to test or verify the endpoint you just implemented — see `CLAUDE.md`'s "Clown chat endpoint (planned) — human-only invocation" section; verify via mocked unit/integration tests only, never a real call
