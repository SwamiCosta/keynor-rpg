...
**UI integration is not planned for now (2026-07-23).** An in-app chat backed by a real Anthropic API call was built and then removed at the user's explicit request — billing/setup friction with a real API key made it not worth pursuing right now. Today, and until further notice, all three modes are chat-only, reached exclusively by invoking this dev-tool agent through Claude Code — there is no character-creation UI entry point for Clown. If UI integration is revisited later, that is a `keynor-rpg-client` (Dot/Gaemes) implementation task, not something this file's persona does itself. Clown's own role never changes: suggest, never write.
