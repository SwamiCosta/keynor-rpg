...
## Rename: MeleeAccuracy → MeleeDexterity (2026-07-20)

Pure rename, no formula/weight change — `getMeleeAccuracy()`/`getMeleeAccuracyBreakdown()` → `getMeleeDexterity()`/`getMeleeDexterityBreakdown()`, `AttributesResponse.meleeAccuracy`/`AttributeBreakdownsResponse`'s `meleeAccuracy` key → `meleeDexterity`, and `kMeleeAccuracyPrecision`/`kMeleeAccuracyAgility`/`kMeleeAccuracyDogEatDog` → `kMeleeDexterityPrecision`/`kMeleeDexterityAgility`/`kMeleeDexterityDogEatDog`. Driven by the new Special Attack Test mechanic (see `special-attack-test.md`), where `MeleeDexterity` (MD) is the attribute rolled for every melee attack's hit-check test (`Tmd = MeleeDexterity - 10 + 1d20`).

## Removal: CloseCombat, LowRangeCombat (2026-07-20)

`getCloseCombat()`/`getCloseCombatBreakdown()`, `getLowRangeCombat()`/`getLowRangeCombatBreakdown()`, `kCloseCombatBellicose`, `kCloseCombatFighting`, `kLowRangeCombatBellicose`, `kLowRangeCombatWeaponPracticing`, `kLowRangeCombatFencing`, and the `closeCombat`/`lowRangeCombat` fields on `AttributesResponse`/`AttributeBreakdownsResponse` are all deleted outright — user-requested removal, superseded by the single `MeleeDexterity` attribute (see the rename above) as the one melee-relevant attribute going forward.

`CombatActionTimeCalculator`'s combat-timing formulas (`JAB`, `BODY_STRIKE`, which read `CloseCombat`; `PIERCING_ATTACK`, `LIGHT_SWING_ATTACK`, `HEAVY_SWING_ATTACK`, `DRAW_MELEE_WEAPON`, `BLOCK`, `DRAW_HEAVY_WEAPON`, which read a same-shaped but separately-named `shortRangeCombat` field that nothing actually fed) all now read `MeleeDexterity` instead — `CombatAttributeInputs`/`CombatActionTimeRequest` collapsed their two `closeCombat`/`shortRangeCombat` fields into one `meleeDexterity` field, and `CombatTimingCoefficients`'s matching weight fields were renamed accordingly (`kJabCloseCombat` → `kJabMeleeDexterity`, `kPiercingAttackShortRangeCombat` → `kPiercingAttackMeleeDexterity`, etc. — same mechanical rename across all 8 affected actions).

**Deferred, documented-not-implemented (2026-07-20, explicit user decision):** the four inputs that used to feed `CloseCombat`/`LowRangeCombat` — the `Bellicose` trait, `Fighting` (training), `WeaponPracticing` (training), and `Fencing` (Knowledge) — are not being wired into `MeleeDexterity`'s own additive formula. Instead they are intended to become **situational modifiers on individual Special Attack Test rolls**, not a permanent additive term on the character sheet's `MeleeDexterity` value — e.g. `Fencing` boosting the Tmd roll specifically for sword-type attacks, not every melee attack. This is design intent only; do not implement it until the Special Attack Test's per-weapon/per-attack-type modifier mechanism is defined (see `special-attack-test.md`'s own "Deferred inputs" note). `Fighting`/`WeaponPracticing`/`Fencing` currently have no formula reading them anywhere in `PlayableCharacter` as of this delta.
